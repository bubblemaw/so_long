...
/*   Updated: 2025/01/17 17:11:28 by maw              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	display_image(t_data *data)
{
	...
	while (y * IMG_H < data->map->height)
	{
		...
		{
			mlx_put_image_to_window(data->mlx, data->win, data->img->b, x * IMG_W, y * IMG_H);
			if (data->map->tab[y][x] == 'P')
				mlx_put_image_to_window(data->mlx, data->win, data->img->p, x * IMG_W, y * IMG_H);
			if (data->map->tab[y][x] == 'E')
				mlx_put_image_to_window(data->mlx, data->win, data->img->e, x * IMG_W, y * IMG_H);
			if (data->map->tab[y][x] == '1')
				mlx_put_image_to_window(data->mlx, data->win, data->img->w, x * IMG_W, y * IMG_H);
			if (data->map->tab[y][x] == 'C')
				mlx_put_image_to_window(data->mlx, data->win, data->img->c, x * IMG_W, y * IMG_H);
			x++;
		}
		y++;
	}
