...
/*   Updated: 2024/10/19 21:32:24 by masase           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	{
		stash[fd] = freestash(stash[fd]);
		return (NULL);
	}
	stash[fd] = reading(fd, stash[fd], temp, buffer);
	if (stash[fd])
	{
		line = feedline(stash[fd]);
		stash[fd] = setupstash(stash[fd], temp);
		if (!line || line[0] == '\0')
		{
			free(line);
			stash[fd] = freestash(stash[fd]);
